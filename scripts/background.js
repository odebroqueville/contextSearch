  ...
  switch (message.action) {
    ...
    case "updateSearchOptions":
      getOptions().then((settings) => {
        ...
        if (logToConsole) {
          console.log(
            `Preferences retrieved from sync storage: ${
            ...
            browser.storage.local
              ...
              .catch((err) => {
                if (logToConsole) {
                  console.error(err);
                  console.log(
                    "Failed to remove search engines from local storage.",
                  );
                }
                reject();
              });
      ...
      .then(() => {
        ...
        if (logToConsole) {
          for (let id in searchEngines) {
            console.log(
              `Search engine: ${id} has been saved to local storage as follows:\n`,
            );
...
async function processSearch(info, tab) {
  ...
  if (
    (contextsearch_openSearchResultsInSidebar &&
      id !== "reverse-image-search") || id === "exif-tags"
  ) {
...
function buildSuggestion(text) {
  ...
  for (let id in searchEngines) {
    if (searchEngines[id].keyword === keyword) {
      ...
      suggestion["description"] = "Search " + searchEngines[id].name + " for " +
        searchTerms;
      if (logToConsole) console.log(JSON.stringify(suggestion));
