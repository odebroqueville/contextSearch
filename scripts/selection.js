...
browser.runtime.onMessage.addListener((message, _ignored, sendResponse) => { // eslint-disable-line no-unused-vars
...
async function ask(url, promptText) {
    ...
    const handleChatInput = async () => {
        ...
        if (url.includes('aistudio.google.com')) {
            ...
            textarea = document.querySelector('div#ask-input') ||
                document.querySelector('[contenteditable="true"]') ||
                document.querySelector('[data-testid="chat-input-textbox"]');
            submit = document.querySelector('button[data-testid="submit-button"]') ||
            ...
            if (!submissionSuccessful) {
                ...
                if (!submissionSuccessful && url.includes('www.perplexity.ai')) {
                    try {
                        ...
                        const alternativeSubmit = document.querySelector('[role="button"][aria-label="Send message"]') ||
                            document.querySelector('button:has(svg)') ||
                            Array.from(document.querySelectorAll('button')).find(btn =>
                                btn.textContent === 'Send' ||
                                btn.innerHTML.includes('svg') ||
                                btn.closest('[data-testid="chat-input-actions"]'));
...
async function handleAltClickWithGrid(e) {
    ...
    try {
        ...
        if (e === null || (options && options.quickIconGrid) || (e && e.altKey)) {
            ...
            if (logToConsole) console.log('❌ Conditions NOT met for displaying grid:', {
                isKeyboardShortcut: e === null,
...
async function createIconsGrid(folderId) {
    ...
    nav.addEventListener('click', e => {
        if (logToConsole) console.log('nav CLICK event detected, target:', e.target.id, 'target tagName:', e.target.tagName, 'navEntered:', navEntered);
