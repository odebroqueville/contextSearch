...
const aiUrls = [chatGPTUrl, googleAIStudioUrl, grokUrl, perplexityAIUrl, poeUrl, claudeUrl, andiUrl];
...
document.addEventListener('keydown', (e) => {
    if (logToConsole) console.log('🔽 Keydown event detected:', { key: e.key, code: e.code, altKey: e.altKey });
    handleKeyDown(e);
});

// Key up event listener
document.addEventListener('keyup', (e) => {
    if (logToConsole) console.log('🔼 Keyup event detected:', { key: e.key, code: e.code, altKey: e.altKey });
    handleKeyUp(e);
});
...
window.addEventListener('blur', () => {
    if (logToConsole) console.log('Window lost focus, resetting keysPressed');
    keysPressed = {};
});

// Also reset when window regains focus, just to be extra safe
window.addEventListener('focus', () => {
    if (logToConsole) console.log('Window gained focus, ensuring keysPressed is reset');
    keysPressed = {};
});

// Handle incoming messages from the background script
browser.runtime.onMessage.addListener((message, _unused, sendResponse) => {
    // eslint-disable-line no-unused-vars
    const action = message.action;
    const data = message.data;

    if (logToConsole) console.log('Message received in content script:', { action, data });
        ...
        try {
            switch (action) {
                case 'getOpenSearchSupportStatus':
                    ...
                    return true;
                case 'getTemplateContext': {
                    // Provide rich template variables for background usage
                ...
                case 'getSearchEngine':
                    try {
                        const result = await getOpenSearchEngine();
                        if (result) {
            ...
            if (logToConsole) console.error('Error handling message:', error);
            sendResponse({ success: false });
            return true;
        }
    };
...
function handleKeyDown(event) {
    if (logToConsole) console.log('📥 handleKeyDown called with event:', event);
    const key = event.key;
    ...
    if (logToConsole)
        console.log('Keydown target check:', {
    ...
    if (event.target.nodeName === 'INPUT' || event.target.isContentEditable || !isKeyAllowed(event)) {
        if (logToConsole) console.log('🚫 Keydown event ignored due to input field or disallowed key');
        return;
    }

    // Store the key state - this works for both regular keys and browser command shortcuts
    keysPressed[key] = code;
    if (logToConsole) console.log('🔑 Updated keysPressed:', JSON.stringify(keysPressed));
...
function getOpenSearchSupportStatus() {
    ...
    const hasOpenSearch = !!document.querySelector('link[type="application/opensearchdescription+xml"]');
    if (logToConsole) console.log(`[OpenSearch] hasOpenSearch: ${hasOpenSearch}`);
    ...
    try {
        ...
        if (logToConsole) {
            if (response && response.success) {
                if (logToConsole) console.log('AI Search popup opened successfully.');
            } else if (response) {
                console.error('Failed to open AI Search popup:', response.error || 'Unknown error');
            } else {
                console.error('Failed to open AI Search popup: undefined response');
            }
        }
    } catch (error) {
        if (logToConsole) {
            console.error('Failed to open AI Search popup:', error);
        }
    }
}
...
// Build the rich template context from the current page
function getTemplateContext() {
    // Plain selection text
    let selection = '';
    try {
    ...
    if (!selection && typeof textSelection === 'string') {
    ...
    let selection_html = '';
    try {
        selection_html = getSelectionHtmlSafe();
    } catch (e) {
        selection_html = '';
    }
    ...
    return { selection, selection_html, page_title, url, host, origin, lang, referrer };
}

// Safely extract HTML for the current selection, with sanitization and length cap
function getSelectionHtmlSafe(maxLen = 8000) {
    const sel = window.getSelection && window.getSelection();
...
async function ask(url, promptText) {
    ...
    const handleChatInput = async () => {
        ...
        if (url.includes('aistudio.google.com')) {
            ...
            // Updated selectors for Perplexity.ai
            textarea =
                document.querySelector('div#ask-input') ||
                document.querySelector('[contenteditable="true"]') ||
                document.querySelector('[data-testid="chat-input-textbox"]');
            submit =
                document.querySelector('button[data-testid="submit-button"]') ||
                document.querySelector('button[aria-label="Submit"]') ||
                document.querySelector('[data-testid="chat-input-actions-send"]');
            ...
            textarea = document.querySelector('[data-testid="text-area-input"]') || document.querySelector('div.ProseMirror[contenteditable="true"]');
            submit = document.querySelector('button[data-testid="send-button"]') || document.querySelector('button[aria-label="Send message"]');
        } else if (url.includes('claude.ai')) {
            textarea = document.querySelector('div[contenteditable="true"]');
        } else if (url.includes('andisearch.com')) {
            // Andi search has a more complex UI - try several possible selectors
            textarea = document.querySelector('div.rcw-input[placeholder="Ask Andi..."]');
            submit = document.querySelector('button.rcw-send[type="submit"]');

            if (logToConsole) console.log('Andi search detected with textarea:', textarea, 'and submit:', submit);

            // If we still can't find the elements, try looking more broadly
            if (!textarea) {
                const inputs = document.querySelectorAll('input[type="text"]');
                if (inputs.length > 0) {
                    textarea = inputs[0]; // Use the first text input we find
                    if (logToConsole) console.log('Found fallback input for Andi:', textarea);
                }
            }
            ...
            if (url.includes('claude.ai')) {
                ...
                if (textarea.tagName === 'DIV') {
                    ...
                    if (url.includes('www.perplexity.ai')) {
                        // Create a new input event specifically for Perplexity's React components
                        ...
                        // Also try setting innerText and triggering paste event as fallbacks
        ...
        if (submit && !submit.disabled) {
            ...
            if (!submissionSuccessful) {
                ...
                // If still not successful and this is Perplexity, try other methods
                if (!submissionSuccessful && url.includes('www.perplexity.ai')) {
                    try {
                        ...
                        if (alternativeSubmit) {
                            ...
                            submissionSuccessful = true;
                            if (logToConsole) console.log('Alternative submit button clicked.');
                        }
                    } catch (error) {
                        if (logToConsole) console.log('Alternative submit methods failed:', error);
                    }
                }
            }
            ...
            // Special case for Perplexity if the button wasn't found
            if (url.includes('www.perplexity.ai')) {
                try {
                    // Use Enter key press as fallback for Perplexity
                    const keyEvent = new KeyboardEvent('keydown', {
                        ...
                        keyCode: 13,
                        which: 13,
                        ctrlKey: true, // Some interfaces use Ctrl+Enter to submit
                    });

                    if (textarea) {
                        textarea.dispatchEvent(keyEvent);
                        if (logToConsole) console.log('Tried fallback Enter key submission for Perplexity');
                    }
                } catch (error) {
                    if (logToConsole) console.log('Fallback submission failed:', error);
                }
            }
        }
    };
    ...
    if (document.readyState === 'complete' && !submissionMade && !window.location.href.includes('#_sidebar')) {
        ...
        // For Perplexity specifically, add a small delay to ensure the UI is fully loaded
        if (url.includes('www.perplexity.ai')) {
            await new Promise((resolve) => setTimeout(resolve, 2000)); // 2 second delay
            if (logToConsole) console.log('Added delay for Perplexity.ai');
        }
        ...
        // For cases where we need to wait for the page to finish loading
        if (logToConsole) console.log('Page not ready, waiting for load...');
        window.addEventListener('load', async () => {
            if (logToConsole) console.log('Page loaded, now handling chat input');
            await new Promise((resolve) => setTimeout(resolve, 1500)); // Give a little extra time after load
            await handleChatInput();
        });
    }
}
...
async function init() {
    ...
    try {
        ...
        if (response && response.success && response.data) {
            ...
            if (logToConsole) console.log('Successfully retrieved stored data');
        } else {
            if (logToConsole) console.log('No stored data found or incomplete data!');
...
function hasContextSearchImage() {
    ...
    return Array.from(images).some((img) => img.src.includes('context-search.svg'));
}
...
async function checkForSelection(e) {
    ...
    if (
        (e === null || (e !== null && (e.key === 'Shift' || (e.type === 'mouseup' && e.button === 0)))) &&
...
async function handleKeyUp(e) {
    ...
    if (!keysPressed || Object.keys(keysPressed).length === 0 || (e.target && e.target.nodeName === 'INPUT') || !isKeyAllowed(e)) {
        if (logToConsole) console.log('handleKeyUp early return - no keys pressed or input field');
        return;
    }

    if (logToConsole) console.log('Keys pressed state:', JSON.stringify(keysPressed));

    // Store all modifier keys pressed in var input
    let input = '';
    ...
    // If input ends with a "+", remove it
    if (input.endsWith('+')) {
...
async function searchForKeyboardShortcut(input) {
    ...
    for (let id in searchEngines) {
        ...
        const keyboardShortcut =
            engine && typeof engine.keyboardShortcut === 'string' ? engine.keyboardShortcut.toLowerCase().replace('ctrl', 'control') : '';

        if (logToConsole && keyboardShortcut) console.log(`Comparing: "${keyboardShortcut}" with "${input.toLowerCase()}"`);

        if (keyboardShortcut && keyboardShortcut === input.toLowerCase()) {
            if (logToConsole) console.log(`Matched keyboard shortcut: ${keyboardShortcut} for engine: ${engine.name}`);
            ...
            if (textSelection) {
                try {
                    ...
                    if (logToConsole) console.error('Failed to update selection before search:', error);
                }
            }

            try {
                await browser.runtime.sendMessage({ action: 'doSearch', data: { id: id } });
            } catch (error) {
                if (logToConsole) console.error('Error sending doSearch message:', error);
            }
...
async function handleAltClickWithGrid(e) {
    ...
    try {
        ...
        // Check if alt+click is disabled
        if (!options || options.disableAltClick) {
            if (logToConsole) console.log('🚫 handleAltClickWithGrid: Alt+Click is disabled or options not loaded');
            return;
        }
        ...
        if (e && e.button === 0) {
            ...
            if (!options.quickIconGrid && !e.altKey) {
                if (logToConsole)
                    console.log('🚫 handleAltClickWithGrid: Aborting - quickIconGrid disabled and no Alt key. Event details:', {
        ...
        if (isIconsGridOpen()) {
            if (logToConsole) console.log('🗂️ Closing existing grid');
            closeGrid();
        }

        if (logToConsole)
            console.log('🎯 Checking grid display conditions:', {
                quickIconGrid: options?.quickIconGrid,
        ...
        // Note: We only use direct e.altKey check to ensure we only respond to actual Alt key presses
        if (e === null || (options && options.quickIconGrid) || (e && e.altKey)) {
            if (logToConsole) console.log('🎉 Conditions met! Displaying Icons Grid...');
            ...
            if (e !== null) {
                ...
                if (logToConsole) console.log('Using selection end position:', { x, y });
            }
            ...
            xPos = x + offX;
            yPos = y + offY;
            if (logToConsole) console.log('📍 Grid position calculated:', { xPos, yPos });

            // Force valid coordinates for keyboard shortcut
            if (!Number.isFinite(xPos) || !Number.isFinite(yPos) || xPos <= 0 || yPos <= 0) {
                xPos = Math.max(100, window.innerWidth / 2); //ensures the horizontal position is at least at the center of the viewport
                yPos = Math.max(100, window.innerHeight / 3); //ensures the vertical position is at least at the top third of the viewport
                if (logToConsole) console.log('🔧 Adjusted grid position for keyboard shortcut:', { xPos, yPos });
            }

            if (logToConsole) console.log('🔥 About to call createIconsGrid...');
            await createIconsGrid('root');
            if (logToConsole) console.log('✨ createIconsGrid completed');
        } else {
            if (logToConsole)
                console.log('❌ Conditions NOT met for displaying grid:', {
                    isKeyboardShortcut: e === null,
                    hasQuickIconGrid: options && options.quickIconGrid,
                    hasAltKey: e && e.altKey,
                    eventType: e?.type,
                    altKeyValue: e?.altKey,
                });
        }
    } catch (error) {
        if (logToConsole) console.error('Error in handleAltClickWithGrid:', error);
    } finally {
        // Reset the processing flag
        window.contextSearchProcessing = false;
    }
}
...
function getSelectionEndPosition() {
    ...
    // Fallback: use the center of the viewport if no valid selection position
    return {
...
async function createIconsGrid(folderId) {
    ...
    if (!searchEngines || !searchEngines[folderId]) {
        if (logToConsole)
            console.warn('Search engines not ready or folder missing:', {
                folderId,
                hasSearchEngines: !!searchEngines,
                searchEnginesKeys: searchEngines ? Object.keys(searchEngines) : [],
            });

        // Try to get fresh search engines
        if (logToConsole) console.log('Getting fresh search engines...');
        const freshSearchEngines = await getFreshStoredData('searchEngines');
        if (freshSearchEngines) {
            searchEngines = freshSearchEngines;
            if (logToConsole)
                console.log('Updated searchEngines:', {
    ...
    // Double-check we have what we need after refreshing data
    if (!searchEngines || !searchEngines[folderId]) {
        if (logToConsole) console.error('Still missing search engines after refresh, creating root folder');
        // Create a minimal root folder as fallback
    ...
    for (const id in searchEngines) {
        if (!searchEngines[id] || searchEngines[id].isFolder) continue;
    ...
    for (const id of searchEngines[folderId].children) {
        if (!id || !searchEngines[id]) continue;
        ...
        if (!id.startsWith('separator-') && searchEngine && (searchEngine.show || searchEngine.isFolder)) {
            ...
            if (isEmpty(searchEngine) || !searchEngine.base64) {
    ...
    // Grid dimensions
    const n = icons.length || 1; // Ensure at least 1 for math
    ...
    // If we have no icons to display, show at least a message
    if (icons.length === 0) {
    ...
    nav.addEventListener('click', (e) => {
        if (logToConsole)
            console.log('nav CLICK event detected, target:', e.target.id, 'target tagName:', e.target.tagName, 'navEntered:', navEntered);
        onGridClick(e, folderId);
    });
...
async function onGridClick(e, folderId) {
    e.preventDefault();
    e.stopPropagation();
    if (logToConsole) console.log('onGridClick called - navEntered:', navEntered, 'target:', e.target.id);
    if (!navEntered) {
        if (logToConsole) console.log('onGridClick: navEntered is false, returning early');
        return;
    }
...
function closeGrid() {
    if (logToConsole) console.log('closeGrid() called');
    let nav = document.getElementById('context-search-icon-grid');
    if (nav) {
        if (logToConsole) console.log('Removing Icons Grid from DOM');
        ...
        if (logToConsole) console.log('Icons Grid not found when trying to close');
    }
}
...
async function getNewSearchEngine(url) {
    ...
    const newIndex = searchEngines['root'].children.length;
